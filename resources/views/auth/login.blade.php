...
@section('content')
    ...
    <div class="my-10">
        ...
        <div class="my-20">
            <form class="max-w-sm" method="POST" action="{{route('auth.authenticate')}}">
                @csrf
                <div class="mb-5">
                    <label for="email" class="block mb-2 text-sm font-medium text-gray-900">Почта</label>
                    <input type="text" name="email" id="description" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required />
                </div>
                <div class="mb-5">
